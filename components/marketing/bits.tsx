...
import { Btn, Check, Container, Eyebrow, TextLink, cn } from "@/components/marketing/ui";
...
export function SectionHead({
  ...
  return (
    <div className={cn(center ? "mx-auto max-w-3xl text-center" : "max-w-3xl", className)}>
      ...
      {sub && <p className={cn("mt-5 text-[17.5px] leading-relaxed", light ? "text-white/75" : "text-ink-muted")}>{sub}</p>}
    </div>
  );
}
  ...
  return (
    <div id={id} className="grid items-center gap-10 py-12 md:grid-cols-2 md:gap-16 md:py-16">
      <div data-reveal className={cn(reverse && "md:order-2")}>
        ...
        <p className="mt-4 max-w-[50ch] text-[16.5px] leading-relaxed text-ink-muted">{text}</p>
