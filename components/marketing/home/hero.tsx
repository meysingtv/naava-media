...
import { Btn, Check, Container, cn } from "@/components/marketing/ui";
import { AppShot, LaptopFrame, PhoneShot } from "@/components/marketing/mockups";
    ...
    <section className="relative overflow-hidden bg-brand text-white">
      ...
      <Container className="relative grid items-center gap-14 pb-28 pt-12 md:grid-cols-[1.02fr_.98fr] md:pb-36 md:pt-20">
        ...
        <div className="relative md:pl-6" data-reveal data-delay="80ms">
          ...
          <div className="absolute -right-2 top-4 flex items-center gap-2.5 rounded-[16px] bg-white px-3.5 py-2.5 text-ink shadow-lift md:-right-4">
            <span className="grid h-9 w-9 place-items-center rounded-[12px] bg-brand-light text-brand">
              <Bell className="h-4 w-4" />
            </span>
            <div className="leading-tight">
              <div className="text-[13px] font-extrabold">Termin bestätigt ✓</div>
              <div className="text-[11.5px] text-ink-muted">Lena · Mo 09:00</div>
            </div>
          </div>
        </div>
      </Container>
    ...
    <div className="relative z-10 -mt-14 md:-mt-16">
      <Container>
        <div className="grid gap-3 rounded-3xl bg-white p-3 shadow-lift sm:grid-cols-2 lg:grid-cols-4" data-reveal>
          {TILES.map((t) => (
            <div key={t.v} className="flex items-center gap-4 rounded-[16px] bg-paper px-5 py-4">
              ...
              <div className="leading-tight">
                ...
                <div className="text-[13px] text-ink-muted">{t.l}</div>
              </div>
            </div>
          ))}
        </div>
...
export function LogoStrip() {
  return (
    <div className="border-b border-line bg-white py-10">
      <Container>
        <p className="text-center text-[13px] font-bold uppercase tracking-[0.14em] text-ink-muted">Gemacht für den deutschen Fahrschulalltag</p>
        ...
        <p className="mt-5 text-center text-[12.5px] text-ink-muted">Den Stand einzelner Schnittstellen besprechen wir transparent im Gespräch.</p>
      </Container>
    </div>
  );
}
