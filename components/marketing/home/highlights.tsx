...
import { SectionHead } from "@/components/marketing/bits";
import { ChatMock } from "@/components/marketing/mockups";
      ...
      <Container className="relative">
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
          <div data-reveal className="grid gap-8 rounded-[28px] border border-white/10 bg-white/5 p-7 md:grid-cols-[1fr_1.1fr] md:p-9">
            ...
            <div className="overflow-hidden rounded-[16px] bg-white text-ink shadow-lift">
              <ChatMock />
            </div>
          </div>
          ...
          <div className="grid gap-5">
            {CARDS.map((c, i) => (
              <div key={c.t} data-reveal data-delay={`${i * 70}ms`} className="flex gap-5 rounded-[24px] border border-white/10 bg-white/5 p-6">
                <span className={cn("grid h-12 w-12 shrink-0 place-items-center rounded-[16px] text-white", c.color)}>
