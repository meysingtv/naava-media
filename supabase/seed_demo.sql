...
-- Einmal im Supabase SQL-Editor ausführen. Befüllt die ERSTE Fahrschule
-- mit Team, Schülern, Fahrzeugen, Fahrstunden, Rechnungen und Theorie.
-- (Kann gefahrlos wieder gelöscht werden – siehe unten.)
-- =====================================================================
do $$
declare
  v_fs uuid;
begin
  select id into v_fs from public.fahrschule order by created_at limit 1;
  if v_fs is null then
    raise exception 'Keine Fahrschule gefunden – bitte zuerst eine Fahrschule anlegen.';
  end if;

  -- Schutz gegen versehentliches doppeltes Einspielen
  if exists (
    select 1 from public.fahrlehrer
    where fahrschule_id = v_fs and vorname = 'Michael' and nachname = 'Wagner'
  ) then
    raise notice 'Demo-Daten scheinen bereits vorhanden – Abbruch.';
    return;
  end if;
...
-- =====================================================================
-- Zum Entfernen der Demo-Daten später (optional):
...
--   (Fahrstunden/Rechnungen/Teilnahmen werden per ON DELETE mit entfernt
--    bzw. auf NULL gesetzt – Theoriestunden ggf. manuell löschen.)
-- =====================================================================
