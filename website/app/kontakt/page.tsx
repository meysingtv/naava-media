      ...
      <Container className="relative grid gap-12 md:grid-cols-[1fr_1.05fr] md:gap-16">
        <div>
          ...
          <div className="mt-9 grid gap-4 sm:grid-cols-2">
            {INFO.map((c) => (
              <div key={c.t} className="rounded-2xl bg-white p-5 shadow-card">
                <span className="grid h-10 w-10 place-items-center rounded-xl bg-brand-light text-brand">
                  <c.icon className="h-5 w-5" />
                </span>
                <div className="mt-3 text-[12px] font-extrabold uppercase tracking-[0.12em] text-muted">{c.t}</div>
          ...
          <p className="mt-6 rounded-2xl bg-white px-5 py-4 text-[14px] text-muted shadow-card">
