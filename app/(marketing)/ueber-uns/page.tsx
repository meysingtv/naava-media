...
import { Container, cn } from "@/components/marketing/ui";
import { CtaBand, SectionHead } from "@/components/marketing/bits";
import { PageHero } from "@/components/marketing/page-hero";
import { Pilot } from "@/components/marketing/home/ablauf";
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {WERTE.map((w, i) => (
              <div key={w.t} data-reveal data-delay={`${i * 60}ms`} className="rounded-3xl bg-paper p-6">
                <span className={cn("grid h-12 w-12 place-items-center rounded-[16px] text-white", w.bg)}>
                  <w.icon className="h-[22px] w-[22px]" />
                </span>
                <h2 className="mt-4 text-[19px] font-extrabold tracking-tight">{w.t}</h2>
                <p className="mt-2 text-[15px] leading-relaxed text-ink-muted">{w.d}</p>
              </div>
            ))}
          </div>
          <p className="mt-8 rounded-[16px] bg-orange-light px-5 py-4 text-[14px] text-orange-dark">
