...
export function PickerRow({
  ...
  const liste: Option[] = erlaubeLeer ? [{ id: "", label: leerLabel ?? "Keine Angabe" }, ...options] : options;
      ...
      <BottomSheet visible={open} onClose={() => setOpen(false)} title={label}>
        <View style={{ alignItems: "center" }}>
          <DateTimePicker
            ...
            display="inline"
            locale="de-DE"
            minimumDate={min ? parseDatum(min) : undefined}
            maximumDate={max ? parseDatum(max) : undefined}
            themeVariant={schema}
            onChange={(_e, d) => {
